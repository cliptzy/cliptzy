...
impl CropStrategy for SplitBrollCrop {
    ...
    fn build_command(
        ...
        let broll_scale = FilterNode::new("scale")
            .param("w", &format!("if(gt(dar\\,{}/{})\\,{}\\,-2)", output_config.width, half_height, output_config.width))
            .param("h", &format!("if(gt(dar\\,{}/{})\\,-2\\,{})", output_config.width, half_height, half_height))
            .inputs(&[&broll_v])
        ...
        log::info!("FFmpeg SplitBrollCrop filter_complex: {}", graph.to_string());
        Ok(builder)
    }
}
