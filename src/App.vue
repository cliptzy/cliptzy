...
<template>
  <div v-if="specs && !specs.meets_requirements" class="fixed inset-0 z-[10000] bg-black/90 backdrop-blur-md flex items-center justify-center p-6">
    <div class="bg-[var(--color-surface)] border border-red-500/50 rounded-2xl p-8 max-w-md w-full shadow-[0_0_40px_rgba(239,68,68,0.2)]">
      <div class="flex items-center justify-center w-16 h-16 rounded-full bg-red-500/20 text-red-500 mb-6 mx-auto">
      ...
      <h2 class="text-xl font-bold text-white text-center mb-2">Spesifikasi Minimal Tidak Terpenuhi</h2>
      <p class="text-sm text-gray-400 text-center mb-6">Aplikasi tidak dapat dilanjutkan karena sistem Anda tidak memenuhi spesifikasi minimal untuk menjalankan proses AI & Rendering.</p>
      
      <div class="bg-black/50 border border-[var(--color-subtle)] rounded-xl p-4 mb-6">
        <ul class="space-y-2">
          <li v-for="reason in specs.missing_reasons" :key="reason" class="flex items-start gap-2 text-sm text-red-400">
