<template>
  <footer class="h-6 w-full bg-[#09090B] border-t border-[#27272A] flex items-center justify-between px-2 text-[10px] text-gray-400 font-mono z-50 shrink-0">
    <div class="flex items-center gap-4 h-full">
      <!-- Auth Status -->
      <div class="flex items-center gap-1.5 hover:text-white cursor-pointer transition-colors h-full px-1" title="Authentication Status">
      ...
      <!-- App Status -->
      <div class="flex items-center gap-1.5 hover:text-white cursor-pointer transition-colors h-full px-1" title="Application Status">
        <template v-if="appStore.isProcessing">
          <svg class="w-3 h-3 animate-spin text-[var(--color-accent)]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <span class="text-[var(--color-accent)]">{{ appStore.progressLabel || 'Processing...' }} ({{ Math.round(appStore.globalProgress) }}%)</span>
        </template>
        <template v-else>
          <svg class="w-3 h-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
          <span>Ready</span>
        </template>
      </div>
    </div>
    ...
    <div class="flex items-center gap-4 h-full">
      ...
      <div class="flex items-center gap-3 h-full">
        <!-- CPU Usage -->
        <div class="flex items-center gap-1 hover:text-white cursor-default px-1" title="CPU Usage">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z"></path></svg>
          <span>{{ cpuUsage }}%</span>
        </div>
        
        <!-- GPU Usage -->
        <div class="flex items-center gap-1 hover:text-white cursor-default px-1" title="GPU Usage">
        ...
        <!-- RAM Usage -->
        <div class="flex items-center gap-1 hover:text-white cursor-default px-1" title="RAM Usage">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
          <span>{{ memoryUsage }} MB</span>
        </div>

        <!-- Network -->
        <div class="flex items-center gap-1 hover:text-white cursor-default px-1" title="Network (Download / Upload)">
      ...
      <!-- Version / Backend -->
      <div class="flex items-center gap-1.5 hover:text-white cursor-pointer transition-colors h-full px-1" title="Native Rust Backend">
      ...
      <!-- Notifications (Bell) -->
      <div class="flex items-center gap-1.5 hover:text-white cursor-pointer transition-colors h-full px-1" title="Notifications">
