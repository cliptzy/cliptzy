<template>
  <footer class="h-6 w-full bg-base-200 flex items-center justify-between px-2 text-[10px] text-base-content/60 font-mono z-50 shrink-0 shadow-[0_-1px_3px_rgba(0,0,0,0.05)]">
    <div class="flex items-center gap-4 h-full">
      <!-- Auth Status -->
      <div class="flex items-center gap-1.5 hover:text-base-content cursor-pointer transition-colors h-full px-1" title="Authentication Status">
        <div class="w-1.5 h-1.5 rounded-full" :class="auth.isLoggedIn ? 'bg-success' : 'bg-error'"></div>
        <span>{{ auth.isLoggedIn ? auth.email || 'Authenticated' : 'Offline' }}</span>
      </div>

      <!-- App Status -->
      <div class="flex items-center gap-1.5 hover:text-base-content cursor-pointer transition-colors h-full px-1" title="Application Status">
        ...
        <template v-else>
          <svg class="w-3 h-3 text-base-content/60" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
          <span>Ready</span>
        </template>
      </div>
    </div>
    ...
    <div class="flex items-center gap-4 h-full">
      ...
      <div class="flex items-center gap-3 h-full">
        <!-- CPU Usage -->
        <div class="flex items-center gap-1 hover:text-base-content cursor-default px-1" title="CPU Usage">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 3v2m6-2v2M9 19v2m6-2v2M5 9H3m2 6H3m18-6h-2m2 6h-2M7 19h10a2 2 0 002-2V7a2 2 0 00-2-2H7a2 2 0 00-2 2v10a2 2 0 002 2zM9 9h6v6H9V9z"></path></svg>
          <span>{{ cpuUsage }}%</span>
        </div>
        
        <!-- GPU Usage -->
        <div class="flex items-center gap-1 hover:text-base-content cursor-default px-1" title="GPU Usage">
        ...
        <!-- RAM Usage -->
        <div class="flex items-center gap-1 hover:text-base-content cursor-default px-1" title="RAM Usage">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
          <span>{{ memoryUsage }} MB</span>
        </div>

        <!-- Network -->
        <div class="flex items-center gap-1 hover:text-base-content cursor-default px-1" title="Network (Download / Upload)">
      ...
      <!-- Version / Backend -->
      <div class="flex items-center gap-1.5 hover:text-base-content cursor-pointer transition-colors h-full px-1" title="Native Rust Backend">
      ...
      <!-- Notifications (Bell) -->
      <div class="flex items-center gap-1.5 hover:text-base-content cursor-pointer transition-colors h-full px-1" title="Notifications">
...
const stageColorClass = computed(() => {
  ...
  switch (stage) {
    case 'download': return 'text-base-content/60';
    case 'crop': return 'text-base-content/60';
    case 'transcribe': return 'text-base-content/60';
    case 'subtitle': return 'text-base-content/60';
    case 'finalize': return 'text-base-content/60';
