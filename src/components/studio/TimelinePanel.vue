<template>
  ...
  <BentoCard class="flex-1 p-6 flex flex-col justify-between">
      <div class="flex items-center justify-between mb-4">
        ...
        <span class="text-xs text-gray-900 dark:text-gray-100 font-mono font-bold tracking-widest bg-gray-300 dark:bg-gray-800 px-3 py-1 rounded-full shadow-sm">
            {{ formatTime(videoStore.currentTime) }} <span class="text-gray-700 dark:text-gray-300">/ {{ formatTime(videoStore.selectedSegment?.end || videoStore.metadata?.duration || 0) }}</span>
        </span>
      </div>
      ...
      >
        ...
        >
          <div class="absolute -top-1 -left-1 w-3 h-3 rotate-45 bg-gray-600 dark:bg-gray-400"></div>
        </div>
        
        <div v-if="!videoStore.metadata" class="w-full text-center text-xs text-gray-700 dark:text-gray-300 font-black uppercase tracking-widest">
          TIDAK ADA MEDIA
        </div>
        
        <div v-else class="w-full h-full flex flex-col justify-center gap-2 relative z-10 pointer-events-none p-2">
          <!-- Zoomed Segment View -->
          <template v-if="videoStore.selectedSegment">
              <!-- Subtitle Track -->
              <div class="h-8 w-full relative flex items-center justify-center bg-gray-100/50 dark:bg-gray-950/50 rounded-lg">
                  <span v-if="videoStore.isAnalyzing" class="text-[10px] text-gray-700 dark:text-gray-300 font-bold uppercase animate-pulse flex items-center gap-1">
                      <IconLoader class="w-4 h-4 animate-spin" /> Transcribing Audio...
                  </span>
                  <template v-else-if="segmentTranscript.length">
                      ...
                      >
                          <span class="text-[9px] text-gray-900 dark:text-gray-100 font-bold px-1 truncate w-full text-center">{{ word.text }}</span>
                      </div>
                  </template>
              </div>

              <!-- Main Video Track -->
              <div class="h-12 w-full bg-gray-500/20 border border-gray-500/30 rounded-xl flex flex-col justify-center px-4 relative overflow-hidden">
                <span class="text-[10px] font-black text-gray-900 dark:text-gray-100 uppercase px-2 z-10">
                ...
                <div class="absolute bottom-0 left-0 h-1.5 bg-gray-500 z-20 pointer-events-none transition-all duration-75" :style="{ width: `${progressPercentage}%` }"></div>
              </div>
          </template>
          
          <!-- Full Video View (All Segments) -->
          <template v-else-if="videoStore.metadata.segments?.length">
              <div class="w-full h-12 relative bg-gray-100/50 dark:bg-gray-950/50 rounded-xl">
                  <div 
                  ...
                  >
                      <span class="text-[9px] font-black text-gray-900 dark:text-gray-100 uppercase truncate" v-if="((seg.end - seg.start) / videoStore.metadata.duration) > 0.05">
                          Klip {{ idx + 1 }}
                      </span>
                  </div>
              </div>
          ...
          <template v-else>
              <div class="h-12 w-full bg-gray-500/20 border border-gray-500/30 rounded-xl flex flex-col justify-center px-4 relative">
                <span class="text-[10px] font-black text-gray-900 dark:text-gray-100 uppercase">RAW VIDEO</span>
              </div>
          </template>
        </div>
      </div>
