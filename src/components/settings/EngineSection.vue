<template>
      ...
      <div class="flex flex-col gap-3">
        <div class="flex justify-between items-center bg-white/50 dark:bg-black/30 p-3 rounded-2xl border-none shrink-0">
          <div class="flex flex-col">
            <span class="text-sm font-bold text-[var(--color-text-main)]">FFmpeg</span>
            <span class="text-xs font-bold" :class="depsStatus.ffmpeg_installed ? 'text-[var(--color-text-muted)] ' : 'text-red-500'">{{ depsStatus.ffmpeg_version }}</span>
          </div>
          <IconCheckCircle v-if="depsStatus.ffmpeg_installed" class="w-5 h-5 text-[var(--color-text-muted)]" />
          <IconXCircle v-else class="w-5 h-5 text-red-500" />
        </div>

        <div class="flex justify-between items-center bg-white/50 dark:bg-black/30 p-3 rounded-2xl border-none shrink-0">
          <div class="flex flex-col">
            <span class="text-sm font-bold text-[var(--color-text-main)]">Deno</span>
            <span class="text-xs font-bold" :class="depsStatus.deno_installed ? 'text-[var(--color-text-muted)] ' : 'text-red-500'">{{ depsStatus.deno_version }}</span>
          </div>
          <IconCheckCircle v-if="depsStatus.deno_installed" class="w-5 h-5 text-[var(--color-text-muted)]" />
          <IconXCircle v-else class="w-5 h-5 text-red-500" />
        </div>
      </div>

      <div v-if="isInstallingDeps" class="flex flex-col gap-2 shrink-0 mt-auto">
        <div class="flex justify-between text-[10px] text-[var(--color-text-main)] font-bold">
  ...
  <BentoCard class="col-span-1 md:col-span-2 xl:col-span-2 row-span-2 h-full overflow-y-auto custom-scrollbar p-6 flex flex-col gap-5 !bg-rose-100 dark:!bg-rose-900/40">
      ...
      <div class="flex flex-col gap-3 shrink-0">
        <span class="text-xs font-bold text-[var(--color-text-main)]">Akselerasi Rendering (FFmpeg)</span>
        <div class="grid grid-cols-2 gap-2">
          <button class="py-3 px-2 rounded-2xl transition-all text-xs font-bold text-center disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!availableAccels.includes('cpu')" :class="settings.config.hw_accel === 'cpu' ? 'bg-[var(--color-accent)] text-white shadow-[0_4px_15px_rgba(232,115,137,0.3)]' : 'bg-white/60 dark:bg-black/30 text-[var(--color-text-main)] hover:bg-white dark:hover:bg-black/50'" @click="settings.config.hw_accel = 'cpu'">
            CPU
          </button>
          <button class="py-3 px-2 rounded-2xl transition-all text-xs font-bold text-center disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!availableAccels.includes('mac')" :class="settings.config.hw_accel === 'mac' ? 'bg-[var(--color-accent)] text-white shadow-[0_4px_15px_rgba(232,115,137,0.3)]' : 'bg-white/60 dark:bg-black/30 text-[var(--color-text-main)] hover:bg-white dark:hover:bg-black/50'" @click="settings.config.hw_accel = 'mac'">
            Mac
          </button>
          <button class="py-3 px-2 rounded-2xl transition-all text-xs font-bold text-center disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!availableAccels.includes('nvidia')" :class="settings.config.hw_accel === 'nvidia' ? 'bg-[var(--color-accent)] text-white shadow-[0_4px_15px_rgba(232,115,137,0.3)]' : 'bg-white/60 dark:bg-black/30 text-[var(--color-text-main)] hover:bg-white dark:hover:bg-black/50'" @click="settings.config.hw_accel = 'nvidia'">
            NVENC
          </button>
          <button class="py-3 px-2 rounded-2xl transition-all text-xs font-bold text-center disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!availableAccels.includes('amd')" :class="settings.config.hw_accel === 'amd' ? 'bg-[var(--color-accent)] text-white shadow-[0_4px_15px_rgba(232,115,137,0.3)]' : 'bg-white/60 dark:bg-black/30 text-[var(--color-text-main)] hover:bg-white dark:hover:bg-black/50'" @click="settings.config.hw_accel = 'amd'">
            AMF
          </button>
        </div>
        <span class="text-[10px] text-[var(--color-text-muted)] font-medium" v-if="isLoadingAccels">Mendeteksi hardware yang tersedia...</span>
      </div>

      <!-- Threads -->
      <div class="flex flex-col gap-2 mt-2 shrink-0">
        <div class="flex justify-between items-center">
          <span class="text-xs font-bold text-[var(--color-text-main)]">Maksimum Worker/Thread</span>
      ...
      <div class="flex flex-col gap-3 pt-5 border-t border-gray-300 dark:border-gray-800 mt-auto shrink-0">
        <span class="text-xs font-bold text-[var(--color-text-main)]">API Keys (AI Analytics)</span>
        <div class="relative group">
          <IconKey class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-700/60 dark:text-gray-300/60" />
          <input type="password" v-model="settings.config.ai.openai_key" placeholder="OpenAI Key (sk-...)" class="w-full bg-white/60 dark:bg-black/30 border-none rounded-2xl py-3 pl-11 pr-4 text-sm font-bold text-[var(--color-text-main)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] placeholder-gray-700/60 dark:placeholder-gray-300/60 transition-all shadow-sm" />
        </div>
        <div class="relative group">
          <IconSparkles class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-700/60 dark:text-gray-300/60" />
          <input type="password" v-model="settings.config.ai.gemini_key" placeholder="Gemini API Key" class="w-full bg-white/60 dark:bg-black/30 border-none rounded-2xl py-3 pl-11 pr-4 text-sm font-bold text-[var(--color-text-main)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] placeholder-gray-700/60 dark:placeholder-gray-300/60 transition-all shadow-sm" />
        </div>
      </div>
    </BentoCard>
  ...
  <BentoCard class="col-span-1 md:col-span-2 xl:col-span-2 row-span-1 h-full overflow-y-auto custom-scrollbar p-6 flex flex-col gap-4 !bg-orange-100 dark:!bg-orange-900/40">
      ...
      <div class="flex flex-col gap-3 shrink-0">
        <span class="text-xs text-[var(--color-text-main)] font-bold">Model Whisper Default</span>
        <select v-model="settings.config.subtitle.whisper_model" class="w-full bg-white/60 dark:bg-black/30 border-none rounded-2xl p-3 text-sm font-bold text-[var(--color-text-main)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)] transition-all shadow-sm">
        ...
        <p class="text-[10px] text-[var(--color-text-muted)] font-medium mt-1 leading-relaxed">Pengaturan tampilan subtitle (font, warna, ukuran) telah dipindahkan ke menu Studio agar Anda bisa melihat perubahannya secara real-time.</p>
      </div>
    </BentoCard>
</template>
